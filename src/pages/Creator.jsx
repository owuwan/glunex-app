...
import { 
  ...
  Image as ImageIcon, Loader2, Camera, Wand2
} from 'lucide-react';
...
절대로 '글루넥스', 'GLUNEX' 또는 특정 브랜드/매장 이름을 언급하지 마세요. 오직 시공 서비스 정보에 집중하세요.
...
const Creator = ({ userStatus }) => {
  ...
  const callFalAI = async (prompt) => {
    ...
    if (!apiKey || apiKey === "undefined") {
      ...
      return `https://placehold.co/800x600/f1f5f9/64748b?text=API+Key+Missing`;
    }
  ...
  const handleGenerateFullContent = async () => {
    ...
    try {
      ...
      // 본문 태그 치환 및 애니메이션 래퍼 추가
      let finalHtml = data.blog_html;
      images.forEach((url, i) => {
        const replacement = `
          <div class="my-10 rounded-[2.5rem] overflow-hidden border border-slate-100 shadow-xl bg-slate-50 animate-fade-in-up">
    ...
    return (
      <div className="flex flex-col h-full bg-white items-center justify-center animate-fade-in p-10 text-center">
        ...
        <div className="space-y-3 px-4">
            <p className="text-sm text-slate-600 font-bold leading-relaxed whitespace-pre-line">{loadingMsg}</p>
  ...
  return (
    <div className="h-full flex flex-col bg-white font-noto overflow-hidden relative text-left">
      ...
      <style>{`
        ...
        .blog-content h2:nth-child(1) { animation-delay: 0.1s; }
        .blog-content p:nth-child(2) { animation-delay: 0.2s; }
      `}</style>
      ...
      <main className="flex-1 overflow-y-auto p-5 space-y-6 pb-40">
          ...
          <>
              ...
              <div className={`p-6 rounded-[2.5rem] border-2 transition-all duration-700 shadow-xl ${isWeatherEnabled ? 'bg-blue-600 border-blue-400 text-white' : 'bg-slate-50 border-slate-100 text-slate-600'}`}>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xs font-black uppercase tracking-widest leading-none">날씨 인사이트 분석</h2>
                  </div>
                ...
                <p className="text-[13px] font-bold opacity-95">
                  {isWeatherEnabled ? `오늘 '${weather.desc}' 날씨에 고객들이 가장 필요로 할 서비스가 무엇인지 분석하여 제안합니다.` : "날씨 상황과 무관한 일반적인 전문 마케팅 원고를 생성합니다."}
                </p>
              </div>
            </section>
            ...
            <section className="space-y-4 animate-fade-in">
              <div className="flex items-center justify-between px-1">
                <h2 className="text-[11px] font-black text-slate-400 tracking-widest uppercase">Select Subjects</h2>
        ...
        {step === 'title' && (
          <section className="space-y-6 animate-fade-in text-left">
            <div className="flex items-center justify-between px-1">
              <div>
                <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded uppercase mb-1 inline-block tracking-tighter">Ai Optimization</span>
                <h2 className="text-xl font-black text-slate-900 tracking-tighter leading-tight italic">제목을 하나만 골라주세요</h2>
              </div>
            ...
            <div className="space-y-3.5">
              {titles.map((t, i) => (
                ...
                >
                  <p className="text-[15px] font-black text-slate-800 leading-tight group-hover:text-blue-600 z-10 relative tracking-tight">{t}</p>
                  <MousePointer2 className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-100 opacity-0 group-hover:opacity-100 transition-all shadow-blue-500" size={24} />
                </button>
              ))}
            </div>
          </section>
        ...
        {step === 'index' && (
          <section className="space-y-8 animate-fade-in">
            <div className="px-1">
              <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded uppercase mb-2 inline-block">Plan Architecture</span>
              <h2 className="text-xl font-black text-slate-900 tracking-tighter leading-tight italic mb-3">설계된 글의 흐름입니다</h2>
              <p className="text-[11px] text-slate-400 font-bold border-l-3 border-blue-600 pl-4 leading-relaxed uppercase tracking-tighter">Selected title will be expanded into 2,500+ characters with 5 premium photos.</p>
            </div>
            
            <div className="bg-slate-50 rounded-[2.5rem] border border-slate-100 p-8 space-y-5 shadow-inner">
               {indexList.map((idx, i) => (
                 <div key={i} className="flex gap-5 items-start group">
                    <div className="w-7 h-7 rounded-xl bg-white border border-slate-200 text-[9px] font-black flex items-center justify-center text-slate-400 shrink-0 group-hover:border-blue-600 group-hover:text-blue-600 transition-all shadow-sm">
                      0{i + 1}
                    </div>
            ...
            <div className="flex gap-3">
              <button onClick={() => handleGenerateIndex(selectedTitle)} className="flex-1 py-5 bg-white border-2 border-slate-100 rounded-3xl text-slate-400 text-xs font-black flex items-center justify-center gap-2 active:scale-95 transition-all">
                <RefreshCw size={16} /> 다시 구성
              </button>
              <button onClick={handleGenerateFullContent} className="flex-[2.2] py-5 bg-slate-900 text-white rounded-3xl text-xs font-black shadow-2xl shadow-slate-900/10 flex items-center justify-center gap-2 active:scale-95 transition-all">
        ...
        {step === 'result' && generatedData && (
          <section className="space-y-6 animate-fade-in pb-20">
            <div className="flex bg-slate-100 p-1 rounded-3xl border border-slate-200 shadow-inner">
              {[{ id: 'blog', name: '블로그' }, { id: 'insta', name: '인스타그램' }, { id: 'short', name: '숏폼대본' }].map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex-1 py-3.5 rounded-[1.2rem] text-[13px] font-black transition-all ${
                    activeTab === tab.id ? 'bg-slate-900 text-white shadow-xl scale-[1.02] z-10' : 'text-slate-400 hover:text-slate-700'
                  }`}
                >
                  {tab.name}
                </button>
            ...
            <div className="bg-white p-8 rounded-[4rem] border border-slate-100 shadow-2xl min-h-[600px] relative overflow-hidden text-left border-t-[10px] border-t-blue-600">
              ...
              <div className="pt-16">
                {activeTab === 'blog' ? (
                  <article className="prose prose-slate max-w-none font-noto blog-content">
                    ...
                    <div className="text-[16px] leading-[2.2] text-slate-600 font-medium space-y-12 tracking-tight" dangerouslySetInnerHTML={{ __html: generatedData.blog_html }} />
                  </article>
                ) : (
                  <div className="pt-8">
                    <div className="bg-blue-50 text-blue-600 p-4 rounded-3xl mb-8 border border-blue-100 flex items-center gap-3 animate-fade-in-up">
                       <Camera size={18} className="animate-pulse" />
                       <p className="text-[11px] font-black uppercase tracking-widest leading-none">iPhone Style Content Optimized</p>
                    </div>
                    <pre className="whitespace-pre-wrap font-noto text-[14.5px] text-slate-800 leading-loose font-bold italic bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 shadow-inner animate-fade-in-up">
              ...
              <div className="mt-24 pt-8 border-t border-slate-50 text-center opacity-30">
                 <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.5em]">Engine v4.2 • High-Performance AI</p>
              </div>
            </div>
          </section>
        )}
      ...
      {/* 하단 고정 버튼 */}
      <footer className="fixed bottom-0 left-0 right-0 p-8 bg-white/80 backdrop-blur-3xl border-t border-slate-100 max-w-md mx-auto z-40 shadow-[0_-10px_30px_rgba(0,0,0,0.03)]">
        {step === 'keyword' && (
        ...
        {step === 'result' && (
           <div className="flex gap-3">
              <button onClick={handleCopy} className="flex-[2.5] py-6 bg-slate-900 text-white rounded-[2.8rem] font-black text-[17px] shadow-2xl active:scale-95 flex items-center justify-center gap-4 transition-all hover:bg-slate-800">
                 {isCopied ? <CheckCircle2 size={24} className="text-green-400"/> : <Copy size={24}/>} {isCopied ? '복사 완료' : '전체 내용 복사'}
              </button>
              <button onClick={() => {
                setStep('keyword');
                ...
                setTitles([]);
                setIndexList([]);
              }} className="flex-1 py-6 bg-white border-2 border-slate-100 rounded-[2.8rem] text-slate-400 font-black text-[14px] active:scale-95 transition-all hover:bg-slate-50">초기화</button>
           </div>
        )}
      </footer>
    </div>
