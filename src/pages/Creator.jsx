    ...
    try {
      ...
      images.forEach((url, i) => {
        const replacement = url ? `
          <div class="my-10 rounded-[2rem] overflow-hidden border border-slate-100 shadow-2xl animate-fade-in-up bg-slate-50">
            ...
            <div class="p-3 bg-white text-center border-t border-slate-50 flex items-center justify-center gap-2">
              ...
              <span class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] italic">Shot on iPhone 15 Pro</span>
            </div>
          </div>
        ` : `<div class="p-8 text-center text-slate-300 text-xs italic">이미지 생성 지연 중...</div>`;
  ...
  return (
    <div className="h-full flex flex-col bg-[#F8F9FB] font-noto overflow-hidden relative text-left">
      <style>{`
        ...
        .blog-body h2, .blog-body p { opacity: 0; animation: fadeInUp 0.7s ease-out forwards; }
      `}</style>

      {toastMsg && (
        <div className="fixed top-12 left-1/2 -translate-x-1/2 z-[9999] animate-bounce w-full max-w-xs px-4">
          <div className="bg-slate-900 text-white px-8 py-4 rounded-[2rem] text-sm font-black shadow-2xl flex items-center justify-center gap-3 border border-slate-700 backdrop-blur-md">
      ...
      <header className="px-6 py-5 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white/95 backdrop-blur-md z-30 shadow-sm">
        <div className="flex items-center gap-4">
          <button 
            ...
            className="p-2 hover:bg-slate-50 rounded-xl transition-all active:scale-90 border border-transparent"
          >
            <ArrowLeft size={22} className="text-slate-500" />
          </button>
          <div className="text-left">
            <h1 className="text-xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">GLUNEX <span className="text-blue-600">AI</span></h1>
            <p className="text-[9px] text-slate-400 font-black uppercase tracking-[0.2em] mt-1.5 leading-none">Marketing Agent Pro</p>
          </div>
        </div>
        <div className="flex items-center gap-2.5 bg-slate-50 px-4 py-2.5 rounded-3xl min-w-[110px] justify-center border border-slate-100 shadow-inner">
          ...
          <span className="text-[10px] font-black text-slate-700 uppercase tracking-tight">{weather.desc} {weather.temp}°C</span>
        </div>
      </header>
        ...
        {loading ? (
          <div className="flex flex-col h-full items-center justify-center animate-fade-in py-24 text-center">
            <div className="relative mb-12">
              <div className="w-24 h-24 border-[6px] border-slate-100 border-t-blue-600 rounded-full animate-spin shadow-sm"></div>
              <Sparkles className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-blue-600 animate-pulse" size={32} />
            </div>
            <h2 className="text-2xl font-black text-slate-900 mb-4 tracking-tighter italic uppercase leading-none">GLUNEX AI Agent</h2>
            <p className="text-[15px] text-slate-600 font-bold leading-relaxed whitespace-pre-line px-6 tracking-tight">{loadingMsg}</p>
          </div>
        ) : step === 'keyword' ? (
          <>
            {/* 날씨 인사이트 섹션 */}
              ...
              <div className={`p-8 rounded-[3rem] border-2 transition-all duration-700 shadow-2xl ${isWeatherEnabled ? 'bg-blue-600 border-blue-400 text-white shadow-blue-200/50' : 'bg-white border-slate-200 text-slate-900'}`}>
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-lg font-black uppercase tracking-tight leading-none">실시간 날씨 인사이트</h2>
                      <p className={`text-[9px] font-black uppercase mt-1.5 tracking-widest ${isWeatherEnabled ? 'text-blue-200' : 'text-slate-400'}`}>Environment Sync Mode</p>
                    </div>
                  </div>
                ...
                <p className="text-[15px] font-bold opacity-95 leading-relaxed tracking-tight border-t border-white/10 pt-5">
                   {isWeatherEnabled ? `오늘 '${weather.desc}' 날씨를 분석하여 고객이 가장 반응할 마케팅 멘트를 자동으로 생성합니다.` : "날씨와 무관하게 모든 상황에 적합한 전문 마케팅 원고를 생성합니다."}
                </p>
              </div>
            </section>
            ...
            <section className="space-y-6 animate-fade-in pb-10">
              <div className="flex items-center justify-between px-2">
                 <h2 className="text-xl font-black text-slate-900 tracking-tighter flex items-center gap-2 uppercase italic">
              ...
              <div className="grid grid-cols-3 gap-3">
                {categories.map((cat) => (
                  <button 
                    ...
                    className={`relative py-8 px-2 rounded-[2.2rem] border-2 transition-all duration-500 text-center ${
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                   <div className="bg-blue-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase shadow-lg tracking-widest flex items-center gap-2">
                      <Sparkles size={12} fill="currentColor" /> Ai Best Selection
                   </div>
                </div>
                <h2 className="text-3xl font-black text-slate-900 tracking-tighter leading-tight italic">제목을 선택하세요</h2>
              </div>
              <button onClick={handleGenerateTitles} className="p-3.5 bg-white border border-slate-100 rounded-2xl text-slate-400 hover:text-blue-600 active:scale-95 shadow-md group">
            ...
            <div className="space-y-4">
              {titles.map((t, i) => (
                <button key={i} onClick={() => handleGenerateIndex(t)}
                  className="w-full text-left p-7 rounded-[2.5rem] bg-white border border-slate-50 hover:border-blue-500 transition-all active:scale-[0.98] group relative overflow-hidden shadow-sm border-l-[10px] border-l-blue-600/10 hover:border-l-blue-600"
                >
          ...
          <section className="space-y-10 animate-fade-in">
            <div className="px-1">
              <span className="text-[11px] font-black text-blue-600 bg-blue-50 px-4 py-1.5 rounded-full uppercase mb-4 inline-block tracking-widest shadow-sm">Content Roadmap</span>
              <h2 className="text-3xl font-black text-slate-900 tracking-tighter leading-tight italic mb-3">설계된 글의 흐름입니다</h2>
              <p className="text-[11px] text-slate-400 font-bold border-l-4 border-blue-600 pl-4 leading-relaxed uppercase tracking-tighter">Every section will be detail-written with 5 premium iPhone photos.</p>
            </div>
            ...
            <div className="flex gap-4">
              <button onClick={() => handleGenerateIndex(selectedTitle)} className="flex-1 py-6 bg-white border-2 border-slate-100 rounded-[2.5rem] text-slate-500 text-sm font-black active:scale-95 transition-all flex items-center justify-center gap-2 hover:bg-slate-50">
                <RefreshCw size={20} /> 재생성
              </button>
              <button onClick={handleGenerateFullContent} className="flex-[2.5] py-6 bg-slate-900 text-white rounded-[2.5rem] font-black text-base shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3">
                <FileText size={22} className="text-blue-400" /> 원고 집필 시작
              </button>
            </div>
          </section>
        ) : (
          <section className="space-y-8 animate-fade-in pb-20">
            {/* 채널 탭 선택 */}
            <div className="flex bg-slate-200/50 p-2 rounded-[2.5rem] border border-slate-100 shadow-inner">
              {[
                ...
                <button 
                  ...
                  className={`flex-1 py-4 rounded-[2rem] text-[13px] font-black flex items-center justify-center gap-2.5 transition-all duration-500 ${
            ...
            {/* 결과 콘텐츠 카드 */}
            <div className="bg-white p-10 rounded-[4rem] border border-slate-100 shadow-2xl min-h-[600px] relative overflow-hidden text-left border-t-[12px] border-t-blue-600 animate-fade-in-up">
              <div className="absolute top-10 right-10 z-30">
                <button onClick={handleCopy} className={`p-5 rounded-[1.8rem] border-2 transition-all active:scale-90 shadow-xl ${isCopied ? 'bg-green-50 border-green-200 text-green-600' : 'bg-slate-900 border-slate-800 text-white'}`}>
                  {isCopied ? <Check size={26} /> : <Copy size={26} />}
                </button>
              </div>
              ...
              <div className="pt-16">
                {activeTab === 'blog' ? (
                  <article className="prose prose-slate max-w-none font-noto blog-body">
                    ...
                    <div className="text-[17px] leading-[2.3] text-slate-600 font-medium space-y-12 tracking-tight" dangerouslySetInnerHTML={{ __html: generatedData.blog_html }} />
                  </article>
                ) : (
                  <div className="pt-10 animate-fade-in-up">
                    <div className="bg-blue-50 text-blue-600 p-5 rounded-[2.5rem] mb-10 border border-blue-100 flex items-center gap-4">
                       <Sparkles size={22} className="animate-pulse" />
                       <p className="text-[13px] font-black uppercase tracking-[0.1em] leading-none">Premium Social Script Pro</p>
                    </div>
              ...
              <div className="mt-24 pt-12 border-t border-slate-50 text-center opacity-30">
                 <p className="text-[11px] text-slate-400 font-black uppercase tracking-[0.6em]">GLUNEX AI Engine v5.1 • Premium Agent</p>
              </div>
            </div>
            
            <button onClick={() => { setStep('keyword'); setGeneratedData(null); setTitles([]); setIndexList([]); }} className="w-full py-7 text-slate-400 text-[12px] font-black flex items-center justify-center gap-4 uppercase tracking-[0.5em] bg-white border border-slate-100 rounded-[2.5rem] hover:bg-slate-50 transition-all active:scale-95 shadow-sm">
      ...
      {/* 하단 고정 액션 바 */}
      <footer className="fixed bottom-0 left-0 right-0 p-8 bg-white/80 backdrop-blur-3xl border-t border-slate-100 max-w-md mx-auto z-40 shadow-[0_-15px_40px_rgba(0,0,0,0.05)]">
        {step === 'keyword' && (
          <button 
            ...
            className={`w-full py-7 rounded-[3rem] font-black text-[18px] flex items-center justify-center gap-5 transition-all active:scale-95 shadow-2xl ${
              selectedTopics.length > 0 ? 'bg-slate-900 text-white shadow-slate-900/40' : 'bg-slate-100 text-slate-300 border border-slate-200'
            }`}
          >
        ...
        {step === 'result' && (
           <div className="flex gap-4">
              <button onClick={handleCopy} className="flex-[2.5] py-7 bg-slate-900 text-white rounded-[3rem] font-black text-[17px] shadow-2xl active:scale-95 flex items-center justify-center gap-5 transition-all">
                 {isCopied ? <CheckCircle2 size={24} className="text-green-400"/> : <Copy size={24}/>} {isCopied ? '복사 완료' : '전체 내용 복사'}
              </button>
              <button onClick={() => setStep('keyword')} className="flex-1 py-7 bg-white border-2 border-slate-900 text-slate-900 rounded-[3rem] font-black text-[15px] active:scale-95 transition-all">초기화</button>
           </div>
        )}
      </footer>
    </div>
